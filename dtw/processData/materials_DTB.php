...
*
*1.CRUD Operations--I will put them just below this comments. They will be triggered by post/GET requests
...
$action="Box Deleted";
$description=" A box has been Deleted";
...
while($tableRow=mysql_fetch_array($tableResult)){
    ...
    $tableData.='<td>'.$contact.'</td>'.'<td>'.$date.'</td>';
      $link='materials_packing.php?package_id='.$package_id;
    if($priv_materials_edit>=3){
       $tableData.='<td><a href="'.$link.'&printPdf=box_summary"><img src="../images/icons/view2.png" height="20px"/></a></td>';
      } 
    if($priv_materials_edit>=3){
       $tableData.='<td><a style="text-decoration:none;"  href="'.$link.'&tab=5"><img src="../images/icons/edit.png" height="20px"/></a></td>';
      } 
