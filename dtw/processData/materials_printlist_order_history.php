...
$action="Vendor print order set As Active";
$description="Vendor ".addslashes($activeVendorName)." Has Been Updated";
...
$id=$_GET["deleteId"];
//Get Vendor details for the log
$sql="select * from materials_printlist_history where id ='$id'";
$resultU=mysql_query($sql);
while($row=mysql_fetch_array($resultU)){
$activeVendorName=$row['vendor_name'];
...
$action="Vendor Quote Deleted";
$description="Vendor ".mysql_real_escape_string($activeVendorName)." Has Been Deleted";
$ArrayData = array($M_module, $action, $description);
quickFuncLog($ArrayData);
