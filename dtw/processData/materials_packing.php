...
$evidenceaction = new EvidenceAction();
  if(isset($_GET['printPdf'])){
    ...
    if($_GET['printPdf']=='summary'){
      ...
      $description=" A Pdf containing the packing information of the active print order has been downloaded";
    ...
    if($_GET['printPdf']=='box_summary' && isset($_GET['package_id'])){
      ...
      $action="Print/Download Pdf";
      $description=" A Pdf containing the packing information of a particular box has been downloaded";
      $ArrayData = array($M_module, $action, $description);
      quickFuncLog($ArrayData);
    }
  }
...
if(isset($_GET["unpackVendor"])==1){
    ...
    mysql_query($sql);
      $action="updated active print order";
      $description=" Update Active Print order as unpacked.General information will be available";
      $ArrayData = array($M_module, $action, $description);
      quickFuncLog($ArrayData);
...
<html xmlns="http://www.w3.org/1999/xhtml" dir="ltr" lang="en-US" xml:lang="en">
 ...
 <div class="clearFix"></div>
    ...
    <div class="contentMain">
          ...
          <h2 align="center">Vendor Packing</h2>
           <div style="margin-left:0%;margin-top:0%;" class="alert alert-block">
				...
				<h4 class="text-center">Warning!</h4>
				   This Form will only display data from the active print order.Before you continue, make sure the active print order 
     ...
     <li <?php if ($tabActive == 'tab1') echo "class='active'" ?>><a href="materials_packing.php?tab=1" >General Packaging Information</a></li>
     <li <?php if ($tabActive == 'tab2') echo "class='active'" ?>><a href="materials_packing.php?tab=2" >Boxes Package Details</a></li>
      ...
      <!---      <li <?php if ($tabActive == 'tab3') echo "class='active'" ?>><a href="#tab3" data-toggle="tab">District Training Boxes</a></li>
         ...
         -->
          ...
          <?php 
             if ($tabActive == 'tab3'){
            require_once("materials_DTB.php"); 
          }
            ?>
      </div>
      ...
      <div class="tab-pane <?php if ($tabActive == 'tab4') echo 'active'; ?>" id="tab4">
          <?php 
            if ($tabActive == 'tab4'){ 
              require_once("materialsAddBox.php");
              } ?>
        </div>
       <div class="tab-pane <?php if ($tabActive == 'tab5') echo 'active'; ?>" id="tab5">
          <?php 
             if ($tabActive == 'tab5'){
               require_once("materialsEditBox.php");
              } 
            ?>
        </div>
      ...
      <div class="tab-pane <?php if ($tabActive == 'tab2') echo 'active'; ?>" id="tab2">
          <?php 
              if ($tabActive == 'tab2') {
                require_once('materials_distribution.php');
              }
...
?>
    ...
    <!-- ================= -->
<?php
$result = mysql_query("SELECT DiSTINCT a.county,a.district_name FROM districts as a,rollout_activity as r  where a.district_name=r.activity_venu ORDER BY a.county");
  ...
  <table class="table table-bordered table-condensed table-striped table-hover">
   ...
   $count=1;
    ...
    while($row=mysql_fetch_array($result)){
      ...
      echo "<tr>";
      echo "<td>".$count."</td><td><input class='texte' type='text' value='".$row["county"]."' readonly name='county".$count."' /></td><td><input class='texte' type='text' readonly  value='".$row["district_name"]."' name='district".$count."' /></td><td><input type='checkbox' class='checkers' name='selection".$count."' value='YES' checked='checked' /></td>";
      echo "</tr>";
      ++$count;
    }
