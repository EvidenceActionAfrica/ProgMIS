...
if(isset($_GET["unpackVendor"])==1){
    ...
    mysql_query($sql);
    header("Location:materials_packing_strict.php#addDistrict");
}
...
<!---    <link href="css/materials.css" rel="stylesheet" type="text/css"/> -->
 ...
 <div class="clearFix"></div>
    <!---------------- content body ------------------------>
    <div class="contentMain" style='margin-left:10%;'>
     ...
     <li <?php if ($tabActive == 'tab2') echo "class='active'" ?>><a href="#tab2" data-toggle="tab">Materials Distribution</a></li>
...
if(isset($_POST["addDistrict"])){
         ...
         $updateResult="Sub-County for Packaging have been selected.";
 ...
 //    echo "District ".$_POST["district".$count]."Selected<br/>".$_POST["selection".$count];
    if($_POST["district".$count] !=null){        
       ...
       $resultP=mysql_query($sql);
        ...
        $updateResult="Sub-County Added Successfully";
...
<form  method="post">
  ...
  <table class="table table-bordered table-condensed table-striped table-hover">
   ...
   $count=1;
    ...
    while($row=mysql_fetch_array($result)){
      ...
      echo "<tr>";
      echo "<td>".$count."</td><td><input class='texte' type='text' value='".$row["county_name"]."' readonly name='county".$count."' /></td><td><input class='texte' type='text' readonly  value='".$row["district_name"]."' name='district".$count."' /></td><td><input type='checkbox'  name='selection".$count."' value='YES' checked='checked' /></td>";
      echo "</tr>";
      ++$count;
    }
 ...
 ?>
  ...
  <input type="submit" name="addDistrict" class="btn-custom" style="margin-left:30%;" value="Set Districts" />
  <?php } ?>
</form>
