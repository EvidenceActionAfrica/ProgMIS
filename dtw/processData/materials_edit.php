...
if(isset($_GET['edit_material'])){
	$materialId=intval($_GET['edit_material']);
?>
<div class="materialCreate">
<?php
		$sql='SELECT * from materials_desc WHERE id='.$_GET['edit_material'];
		$resultMaterial=mysqli_query($db_mysqli_connection,$sql);
		while($rowMaterial=mysqli_fetch_assoc($resultMaterial)){
		
			$materials=$rowMaterial['materials'];
			$materials_abbv=$rowMaterial['materials_abbv'];
			$formula=$rowMaterial['formula'];
			$formula_desc=$rowMaterial['formula_desc'];
			$material_description=$rowMaterial['material_description'];
			$materials_category=$rowMaterial['material_category'];
			$packet=$rowMaterial['packet'];
			$packet_desc=$rowMaterial['packet_desc'];
			$training_box=$rowMaterial['training_box'];
			$training_box_desc=$rowMaterial['training_box_desc'];
			$var1=$rowMaterial['var1'];
			$var2=$rowMaterial['var2'];
			$var3=$rowMaterial['var3'];
			$var4=$rowMaterial['var4'];
			$var5=$rowMaterial['var5'];
			$perform_mround=$rowMaterial['perform_mround'];
			$packaged=$rowMaterial['packaged'];;
			//Remove the code from the formula

			$formula=str_replace('XE#','',$formula);
		}

		mysqli_free_result($resultMaterial);
		...
		$sql = "SELECT * from divisions where district_name='" . $district_name . "'";
			...
			*/
			...
			$formula1=str_replace('$perform_mround','$perform_mroundXE#',$formula1);
	...
	if( is_numeric($var)){
				 ...
				 $sql='SELECT * from materials_desc WHERE materials_abbv="'.$materials_abbv.'" AND id!='.$materialId;
				 $resultAbbv=mysqli_query($db_mysqli_connection,$sql);
				 $numRows=mysqli_affected_rows($db_mysqli_connection);
				 mysqli_free_result($resultAbbv);
			 	if($numRows==0){
					$sql='UPDATE `materials_desc` SET `materials`="'.$materials.'",`materials_abbv`="'.$materials_abbv.'",';
					$sql.='`formula`="'.$formula1.'",`formula_desc`="'.$formula_desc.'",`material_description`="'.$material_description.'",`packet`="'.$packet.'",';
					$sql.='`packet_desc`="'.$packet_desc.'",`material_category`="'.$materials_category.'",`training_box`="'.$training_box.'",';
					$sql.='`training_box_desc`="'.$training_box_desc.'",';
					$sql.='`var1`='.$var1.',`var2`='.$var2.',`var3`='.$var3.',`var4`='.$var4.',`var5`='.$var5.',packaged='.$packaged.',';
					$sql.='`perform_mround`='.$perform_mround.' WHERE `id`='.$materialId;
					mysqli_query($db_mysqli_connection,$sql)or die(mysqli_error($db_mysqli_connection));
					$formulaScore='The Formula Passed.The material has been Updated';
					$action="Updating the Material";
				$description="Updated the material called ".$materials;
				}else{
				$formulaScore='The material\'s abreviation is not unique.Check both then try Again';
				$action="Updating the Material Failed";
				$description="Failed to update the material called ".$materials;
				}
			}else{
...
<div>
	...
	<h2 style='margin-left:30%;'>Update A Material </h2><br/>
		...
		<h4 style='background-color:#bada66;'><?php echo $formulaScore; ?></h4>
			<h2 style='margin-left:20%;'>--------------------- SECTION 1:Basic Information ---------------------</h2>
				...
				<div style='float:left;margin-left:10%;'>
					...
					<label for='material_category'>Material Category</label><br/>
						...
						<?php
							...
							$resultPacket=mysqli_query($db_mysqli_connection,$sql);
							while($row=mysqli_fetch_assoc($resultPacket)){
								if($row['name']==$materials_category){
				...
				<div style='margin-left:50%;'>
					<input type='submit' class='btn-custom' name='updateMaterial' Value='Update Material'/>
