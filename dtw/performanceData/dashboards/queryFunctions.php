...
function numFlexible_2($field, $table, $where, $value, $where1, $value1) {
...
function numFlexible_2_Notsp() {
  $query = "SELECT p_sch_id FROM p_bysch WHERE p_sch_type != 'Private' AND p_sch_type != 'Public' AND p_sch_type != 'Other' AND p_sch_closed = 'No'";
  $result = mysql_query($query) or die(mysql_error());
  $num = mysql_num_rows($result);
  return $num;
}

function num_no_school_p() {
  $query = "SELECT 'p_sch_name' FROM p_bysch WHERE p_sch_type != 'Public' AND p_sch_type != 'Private' AND p_sch_type != 'Other' AND p_sch_closed= 'No'";
  $result = mysql_query($query) or die(mysql_error());
...
function num_no_school_p_Donor($value) {
  $query = "SELECT 'p_sch_name' FROM p_bysch_with_donor WHERE p_sch_type != 'Public' AND p_sch_type != 'Private' AND p_sch_type != 'Other' AND p_sch_closed= 'No' AND donor = '$value'";
  $result = mysql_query($query) or die(mysql_error());
...
function numAttntAv1($field, $table, $where, $value) {
...
function numAttntAv($field, $table, $where, $value, $where1, $value1) {
  $query = "SELECT DISTINCT($field)  FROM $table WHERE $where = '$value' OR $where1 = '$value1'";
  $result = mysql_query($query) or die("<h1>Cannot get num of " . $field . "</h1>" . mysql_error());
...
//performed Deworming
function numJoin1() {
  $query = "SELECT s_prog_sch_id FROM s_bysch INNER JOIN p_bysch WHERE s_bysch.s_prog_sch_id = p_bysch.p_sch_id AND s_bysch.s_deworming_day = p_bysch.p_deworming_day";
...
function numSchoolTypeNotSpecifiedDonor($value) {
  $query = "SELECT p_sch_id FROM p_bysch_with_donor WHERE p_sch_bilharzia = 'Y' AND p_sch_closed = 'No' AND donor = '$value' 
            AND p_sch_type != 'Public'
            AND p_sch_type != 'Private'
            AND p_sch_type != 'Other'   ";
  $result = mysql_query($query) or die("<h1>Cannot get num of p_sch_id</h1>" . mysql_error());
  $num = mysql_num_rows($result);
  return $num;
}

function numSchoolTypeNotSpecified() {
  $query = "SELECT p_sch_id FROM p_bysch WHERE p_sch_bilharzia = 'Y' AND p_sch_closed = 'No' 
            AND p_sch_type != 'Public'
            AND p_sch_type != 'Private'
            AND p_sch_type != 'Other'   ";
  $result = mysql_query($query) or die("<h1>Cannot get num of p_sch_id</h1>" . mysql_error());
...
function numDistinctP_where($field, $treatment, $where, $value) {
  $query = "SELECT DISTINCT($field) FROM p_bysch WHERE p_sch_bilharzia = '$treatment' AND $where = '$value' ";
  $result = mysql_query($query) or die("<h1>Cannot get distinct" . $field . "</h1>" . mysql_error());
  $num = mysql_num_rows($result);
  return $num;
}

function numDistinctP_whereDonor($field, $treatment, $where, $value, $where1, $value1) {
  $query = "SELECT DISTINCT($field) FROM p_bysch_with_donor WHERE p_sch_bilharzia = '$treatment' AND $where = '$value' AND $where1 = '$value1'";
  $result = mysql_query($query) or die("<h1>Cannot get distinct" . $field . "</h1>" . mysql_error());
  $num = mysql_num_rows($result);
  return $num;
}
...
function numDonorOpenSch($field, $table, $where, $value, $where1, $value1) {
  $query = "SELECT $field FROM $table WHERE $where = '$value' AND $where1 = '$value1' ";
  $result = mysql_query($query) or die("<h1>Cannot get " . $field . "</h1>" . mysql_error());
...
function sumDonorOpenSch($field, $table, $where, $value, $where1, $value1) {
  $query = "SELECT SUM($field) AS donor_value FROM $table WHERE $where = '$value' AND  $where1 = '$value1'";
  $result = mysql_query($query) or die("<h1>cannot get count of" . $field . "</h1>" . mysql_error());
...
function sumEstimated($field, $treatment) {
  $query = "SELECT SUM($field)  AS sumEstimated FROM  p_bysch 
            WHERE  p_sch_bilharzia='$treatment'";
...
function sumEstimatedDonor2($field, $treatment, $where, $value) {
  $query = "SELECT SUM($field) AS sumEstimated FROM  p_bysch 
             WHERE  p_sch_bilharzia='$treatment' AND  $where='$value' ";
  $result = mysql_query($query) or die("<h1>cannot get Sum setimated</h1>" . mysql_error());
...
function sumEstimatedDonorOpenSch($field, $treatment, $where, $value, $where1, $value1) {
  $query = "SELECT SUM($field) AS sumEstimated FROM  p_bysch_with_donor 
             WHERE  p_sch_bilharzia='$treatment' AND  $where='$value' AND $where1='$value1'";
  $result = mysql_query($query) or die("<h1>cannot get Sum setimated</h1>" . mysql_error());
...
function hunderdPercentProp() {
  $query = "SELECT s1_school_name FROM s_bysch WHERE s_total_treated/s_total_registered>=1";
  $result = mysql_query($query) or die(mysql_error());
...
function ninetyfivePercentProp() {
  $query1 = "SELECT s1_school_name FROM s_bysch WHERE s_total_treated/s_total_registered>=0.95";
  $result1 = mysql_query($query1) or die(mysql_error());
  $num1 = mysql_num_rows($result1);
  return $num1;
}

function seventyfivePercentProp() {
  $query = "SELECT s1_school_name FROM s_bysch WHERE s_total_treated/s_total_registered>=0.75";
  $result = mysql_query($query) or die(mysql_error());
...
function fiftyPercentProp() {
  $query = "SELECT s1_school_name FROM s_bysch WHERE s_total_treated/s_total_registered>=0.5";
  $result = mysql_query($query) or die(mysql_error());
...
function reporting_on_school_districtDonor($value) {
  $query1 = "SELECT * FROM a_bysch_inner_s_bysch_districtdonor  WHERE rangecheck='1' AND donor='$value'";
  $result1 = mysql_query($query1) or die(mysql_error());
  $districtsInRange = mysql_num_rows($result1);

  $query2 = "SELECT * FROM a_bysch_inner_s_bysch_districtdonor WHERE donor='$value' ";
  $result2 = mysql_query($query2) or die(mysql_error());
  $totaldistricts = mysql_num_rows($result2);

  $ans = round($districtsInRange / $totaldistricts * 100, 2);
  return $ans . '%';
}

function reporting_on_school_divisionDonor($value) {
  $query1 = "SELECT * FROM a_bysch_inner_s_bysch_divisiondonor  WHERE rangecheck='1' AND donor='$value'";
  $result1 = mysql_query($query1) or die(mysql_error());
  $divisionsInRange = mysql_num_rows($result1);

  $query2 = "SELECT * FROM a_bysch_inner_s_bysch_divisiondonor WHERE donor='$value' ";
  $result2 = mysql_query($query2) or die(mysql_error());
  $totalDivisions = mysql_num_rows($result2);
...
function districts_joind_SAD() {
  $query = "SELECT DISTINCT a.district_id FROM a_bysch a, d_bysch d WHERE a.district_id = d.district_id";
  $result = mysql_query($query) or die(mysql_error());
  $num = mysql_num_rows($result);
  return $num;
}

function divsions_joind_SAD() {
  $query = "SELECT DISTINCT a.division_id FROM a_bysch a, d_bysch d WHERE a.division_id = d.division_id";
  $result = mysql_query($query) or die(mysql_error());
  $num = mysql_num_rows($result);
  return $num;
}

function districts_joind_SAD_Donor($value) {
  $query = "SELECT DISTINCT a.district_id FROM a_bysch_with_donor a, d_bysch d WHERE a.district_id = d.district_id AND donor = '$value'";
  $result = mysql_query($query) or die(mysql_error());
  $num = mysql_num_rows($result);
  return $num;
}

function divsions_joind_SAD_Donor($value) {
  $query = "SELECT DISTINCT a.division_id FROM a_bysch_with_donor a, d_bysch d WHERE a.division_id = d.division_id AND donor = '$value'";
  $result = mysql_query($query) or die(mysql_error());
  $num = mysql_num_rows($result);
  return $num;
}

function gok_district() {
  $query1 = "SELECT DISTINCT attnt_district_id FROM attnt_bysch_with_donor WHERE donor = 'CIFF'";
  $result1 = mysql_query($query1) or die(mysql_error());
  $num1 = mysql_num_rows($result1);

  $query2 = "SELECT DISTINCT attnt_district_id FROM attnt_bysch_with_donor WHERE donor = 'END'";
  $result2 = mysql_query($query2) or die(mysql_error());
  $num2 = mysql_num_rows($result2);
  $num = 6 * $num1 + 4 * $num2;
  return $num;
...
function gok_division() {
  $query1 = "SELECT DISTINCT attnt_division_id FROM attnt_bysch_with_donor WHERE donor = 'CIFF'";
  $result1 = mysql_query($query1) or die(mysql_error());
  $num1 = mysql_num_rows($result1);

  $query2 = "SELECT DISTINCT attnt_division_id FROM attnt_bysch_with_donor WHERE donor = 'END'";
  $result2 = mysql_query($query2) or die(mysql_error());
  $num2 = mysql_num_rows($result2);
  $num = 6 * $num1 + 4 * $num2;
  return $num;
...
function forms_within_3months() {
  $numDistricts = 0;

  $sql_query1 = "SELECT DISTINCT district_name,deworming_date FROM a_bysch ORDER BY district_name ASC";
  $result1 = mysql_query($sql_query1) or die(mysql_error());
  while ($rows1 = mysql_fetch_array($result1)) {
    $district_name_a = $rows1['district_name'];
    $deworming_date_a = $rows1['deworming_date'];
    $date1 = date_create($deworming_date_a);


    $sql_query2 = "SELECT DISTINCT district_name,form_SAD_received FROM d_bysch WHERE district_name ='$district_name_a'  ";
    $result2 = mysql_query($sql_query2) or die(mysql_error());
    while ($rows2 = mysql_fetch_array($result2)) {
      $form_SAD_received = $rows2['form_SAD_received'];
      $date2 = date_create($form_SAD_received);
    }

    $diff = date_diff($date2, $date1);
    if ($deworming_date_a == null || $form_SAD_received == null) {
      $deworming_date_a = null;
      $form_SAD_received = null;
      $daysDiff = null;
    } else {
  ...
  $totalDistricts = numDistinctPlain('district_id', 'a_bysch');
  
//  $query = "SELECT DISTINCT($field) FROM $table";
//  $result = mysql_query($query) or die("<h1>Cannot get distinct" . $field . "</h1>" . mysql_error());
//  $totalDistricts = mysql_num_rows($result);
  
  $num = ($numDistricts / $totalDistricts) * 100;
  $returnValue = $num . '%';
