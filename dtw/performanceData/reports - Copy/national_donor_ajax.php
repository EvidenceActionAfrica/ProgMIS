...
if (isset($_POST['national_bar_data'])) {
  ...
  if ($info_type == 'schools_participated') {
    ...
    ///hardcode ///////////////////////////////////////
    if ($donor_selected == "CIFF") {
      $school_targeted = "10000";
    } else {
      $school_targeted = "1000";
    }
    ///hardcode ///////////////////////////////////////
  ...
  if ($info_type == 'children_dewormed') {
    ...
    // //hard coded  ///////////////////////////////
    if ($donor_selected == "CIFF") {
      $children_targeted = "5500000";
    } else {
      $children_targeted = "200000";
    }
    ////////////////////////////////////////
  ...
  if ($info_type == 'children_dewormed_schi') {
    ...
    // //hard coded  ///////////////////////////////
    if ($donor_selected == "CIFF") {
      $pzq_children_targeted = "400000";
    } else {
      $pzq_children_targeted = "200000";
    }
    /////////////////////////////////////////////////
  ...
  // get the schi dewormed data (fifth bar)

  if ($info_type == 'districts_completed_schi') {

    // count districts participated
    $districts_participated = districtsDewormedByDonor($donor_selected, 'PZQ');

    //  count districts PLanned
    $districts_planned = districtsPlannedByDonor($donor_selected, 'PZQ');
...
if (isset($_POST['national_table_data'])) {
  ...
  $pzq_under_5 = "N/A";
  $pzq_over_5 = sumArgsByDonor('a_bysch', $donor_selected, 'ap_6_f', 'ap_11_f', 'ap_15_f', 'ap_6_m', 'ap_11_m', 'ap_15_m','ap_trt_total','ap_ecd_total');
  ...
  $district_num = districtsByDonorSum($donor_selected, 'STH');
  $division_num = divisionByDonorSum($donor_selected, 'STH');
  $sth_district_division_moe = (3 * $district_num) + (4 * $division_num);
  $sth_district_division_mophs = $sth_district_division_moe;

  $district_num_pzq = districtsByDonorSum($donor_selected, 'PZQ');
  $division_num_pzq = divisionByDonorSum($donor_selected, 'PZQ');

  $pzq_district_division_moe = (3 * $district_num_pzq) + (4 * $division_num_pzq);
  $pzq_district_division_mophs = $pzq_district_division_moe;


  $completed_teacher_sessions = getTeacherTrainingSessionDonor('attnt_sth', $donor_selected);
  $pzq_completed_teacher_sessions = getTeacherTrainingSessionDonor('attnt_schisto', $donor_selected);

  $est_number_teachers_trained = getAttntTeachersDonor('attnt_sth', $donor_selected);
  $pzq_est_number_teachers_trained = getAttntTeachersDonor('attnt_schisto', $donor_selected);
...
//			$sth_district_division_mophs++;
  ...
  echo '<!-- --------------------------- First Table --------------------------- -->
			...
			<table border="1" cellpadding="1" cellspacing="1" width="50%" align="center" class="national_donor_report_table">

			<tr><th>Statistics</th><th>STH</th><th>Schisto</th></tr>

			<tr><td>Districts completed</td><td class="left-align">' . $districts_participated . '</td><td class="left-align">' . $pzq_districts_completed . '</td></tr>

			<tr><td>Schools participated</td><td class="left-align">' . number_format($school_participated) . '</td><td class="left-align">' . number_format($pzq_schools_participated) . '</td></tr>

			<tr><td>Children dewormed</td><td class="left-align">' . number_format($children_dewormed) . '</td><td class="left-align">' . number_format($pzq_children_dewormed) . '</td></tr>
	 	...
	 	<table border="1" cellpadding="1" cellspacing="1" width="50%" align="center" class="national_donor_report_table">
			...
			<tr><td>Est.district/division MoE personnel trained</td><td class="left-align">' . $sth_district_division_moe . '</td><td class="left-align">' . $pzq_district_division_moe . '</td></tr>

			<tr><td>Est.district/division MoPHS personnel trained</td><td class="left-align">' . $sth_district_division_mophs . '</td><td class="left-align">' . $pzq_district_division_mophs . '</td></tr>
