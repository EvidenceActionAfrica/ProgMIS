...
function divisionByDonorSum($donor_selected,$treatment){

	if ($treatment=='STH') {
		$result=mysql_query("SELECT COUNT(DISTINCT a_bysch.division_id) AS district_count
							FROM a_bysch 
							LEFT JOIN districts 
							ON a_bysch.district_id = districts.district_id 
							WHERE districts.donor = '$donor_selected'") OR die("CANNOT GET district COUNT".$donor_selected." p_bysch<br>".mysql_error());
		...
		$result=mysql_query("SELECT COUNT(DISTINCT a_bysch.division_id) AS district_count
							...
							LEFT JOIN districts 
							ON a_bysch.district_id = districts.district_id 
							WHERE districts.donor = '$donor_selected' AND ap_attached='Yes'") OR die("CANNOT GET district COUNT".$donor_selected." p_bysch<br>".mysql_error());
		return mysql_num_rows($result);
...
function districtsByDonorSum($donor_selected,$treatment){
...
// get the teacher trainings
	...
	function getAttntTeachersDonor($where,$donor_selected){
		$query="SELECT school_no FROM attnt_bysch
			LEFT JOIN districts 
			ON attnt_bysch.attnt_district_id = districts.district_id 
			WHERE districts.donor = '$donor_selected'
			AND t2_name !='' AND $where='1'";
		$result=mysql_query($query) or die("<h1>Cannot get num of teachers</h1>".mysql_error());
		$num= mysql_num_rows($result);

		$query2="SELECT school_no FROM attnt_bysch
			LEFT JOIN districts 
			ON attnt_bysch.attnt_district_id = districts.district_id 
			WHERE districts.donor = '$donor_selected'
			AND t1_name !='' AND $where='1'";
	...
	*/
	...
	function getTeacherTrainingSessionDonor($where,$donor_selected){
		$query="SELECT school_no FROM attnt_bysch
			LEFT JOIN districts 
			ON attnt_bysch.attnt_district_id = districts.district_id 
			WHERE districts.donor = '$donor_selected'
			AND t2_name !='' AND $where='1' GROUP BY survey_id";
