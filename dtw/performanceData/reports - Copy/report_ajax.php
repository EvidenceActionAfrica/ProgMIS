...
<?php
	...
	if($_REQUEST['checkval']=='school') {
		...
		foreach($insertformdata as $insertformdatav) {
			$tablenames = 'schools';
			$fieldss = 'school_id, school_name';
			$wheres = 'school_id="'.$insertformdatav['school_programme_id'].'"';
			$insertformdatas = $evidenceaction->selectrow($tablenames, $fieldss, $wheres);
		?>
			<option value="<?php echo $insertformdatas['school_id'];?>"><?php echo $insertformdatas['school_name'];?></option>
		<?php }
		die();
	}
