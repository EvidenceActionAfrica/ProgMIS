...
function sumArgsByDistrictOpenSchools(){
	...
	// then add and store the amount gotten in the variable total
	for ($i=0; $i < $size; $i++) { 
		$total+=sumPlainByDistrictOpenSchools($args[$i],$table,$district);
	}
...
function sumPlainByDistrictOpenSchools($field,$table,$district){
	$query="SELECT SUM($field) AS dewormed 
			FROM $table
			WHERE district_id = '$district' AND p_sch_closed= 'No' ";
	$result=mysql_query($query) or die("<h1>cannot get count of".$field."</h1>".mysql_error());
...
function sumMoreFexibleByDistrict2($field,$table,$where,$value,$and,$value2,$and2,$value3){
	$query="SELECT SUM($field) AS dewormed 
			FROM $table
			WHERE $where = '$value'
			AND $and = '$value2'
...
function CountMoreFlexible2($field,$table,$where,$value,$and,$value2,$and1,$value3){
	$query="SELECT COUNT($field) AS number FROM $table WHERE $where = '$value' AND $and = '$value2' AND $and1 = '$value3'     ";
	$result=mysql_query($query) or die("<h1>Cannot get num of ".$field."</h1>".mysql_error());
