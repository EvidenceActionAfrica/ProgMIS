<?php 
	...
	*/
	...
	class ntd extends connDB
	{
		...
		*/
		function runQuery(){
			$rows = $this->getAllDistricts(); // get all distircts
			...
			foreach ($rows as $key => $value) {
				$schools_treated = $this->num('school_id','a_bysch',$value['district_id']);
				$u5_treated = $this->sumUnder5($value['district_id']);
				$sac_treated = 0;
				$over_15_treated = $this->sumArgs('a_bysch',$value['district_id'],'a_15_m','a_15_f');
				$u5_male_treated = $this->sumUnder5Male($value['district_id']);
				$u5_female_treated = $this->sumUnder5Female($value['district_id']);
				$sac_male_treated = 0;
				$sac_female_treated = 0;
				$over_15_male_treated = $this->sumPlain('a_15_m','a_bysch',$value['district_id']);
				$over_15_female_treated = $this->sumPlain('a_15_f','a_bysch',$value['district_id']);
				$adults_treated = $this->sumAdultsFormS($value['district_id']);
				$target_u5 = $this->sumArgs('p_bysch',$value['district_id'],'p_ecd_enroll','p_ecd_sa');
				$target_sac = 0;
				$target_adult = 0;
				...
				$data[] = array(
					...
					'u5_treated' => $u5_treated,
					'sac_treated' => $sac_treated,
					'over_15_treated' => $over_15_treated,
					'u5_male_treated' => $u5_male_treated,
					'u5_female_treated' => $u5_female_treated,
