...
<?php
    ...
    // $type="Public";
    function checkSchool($type){
        $query="SELECT school_name FROM schools WHERE school_type='$type'";
        $result=mysql_query($query) or die("<h1>Cannot get school</h1>".mysql_error());
        $data=array();
        while ($row=mysql_fetch_assoc($result)) {
            $data[]= $row['school_name'];               
        }
        $fields="school_name ='".implode("'  || school_name='",$data)."'";
    ...
    if($_REQUEST['checkval']=='form_s'){
        ...
        // if(isset($_REQUEST['public']) && !empty($_REQUEST['public']) && ($_REQUEST['public']=='public')){
            
        //     $schools=checkSchool('Public');
        //     $fields .= ','.schools.',';
...
legend: {
    enabled: true,
    pointFormat: 'Number: <span style="color:{#0AF}">{point.name6}</span><br />Total: <span style="color:{point.color}">{point.name5}</span><br />'
},
plotOptions: {
    series: {
        borderWidth: 0,
