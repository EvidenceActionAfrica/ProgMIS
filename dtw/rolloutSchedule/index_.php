<?php
	...
	include('includes/rollout.class_.php');
?>
<!DOCTYPE html>
...
<?php
    ...
    //     )
    // );
    // $prevs = $database->statement->fetchall(PDO::FETCH_ASSOC);
        ...
        if (isset($_REQUEST['waveid'])) {
            ...
            if (isset($_REQUEST['add_rollout_activity'])) {
                ...
                if ($validated_data === false) {
                    ...
                    $rollOut->addWaveActivity($validated_data,$_REQUEST['waveid']);
                }
            }

            if (isset($_REQUEST['add_county_activity'])) {
                include('includes/gump.class.php');
                $gump = new GUMP();
                $_POST = $gump->sanitize($_POST);

                $gump->validation_rules(array(
                    'activity_1_start' => 'required'
                ));

                $validated_data = $gump->run($_POST);

                if ($validated_data === false) {
                    $msg1 = array('type' => 0, 'txt' => $gump->get_readable_errors(true));
                } else {
                    $rollOut->addcountyActivity($validated_data,$_REQUEST['waveid']);
                }
            }

            if (isset($_REQUEST['bulk_district_dates'])) {

                $rollOut->bulkAddDistricts($_POST,$_REQUEST['waveid']);
            ...
            <div class="row">
                <div class="col-md-11">
                    ...
                    <div id="messageBox"><?php if (isset($msg1)) {echo $msg1['txt']; } ?></div>

                    <table class="table table-bordered">  
                        <thead>
                            <tr>
                                <th>County</th>
                                <th>County Meetings Planning</th>
                                <th></th>
                            </tr>
                        </thead>              
                        <?php
                                ...
                                <tr>
                                    <form action="<?php basename( $_SERVER['REQUEST_URI'] ) ?>" method="post"> 
                                        ...
                                        <td class="datepicker_input">                                
                                            <input name="county" type="hidden" value="<?php echo $value; ?>" />
                                            <input name="activity_1_start" class="date" type="text" placeholder="Start Date"/>
                                            &nbsp;-&nbsp;
                                            <input name="activity_1_end" class="date" type="text" placeholder="End Date"/>
                                        </td>
                                        <td><button type="submit" class='btn btn-primary' name="add_county_activity"><span class="glyphicon glyphicon-floppy-disk"></span></button></td>
                                    </form>
                                </tr>
                    ...
                    <div id="messageBox"><?php if (isset($msg)) {echo $msg['txt']; } ?></div> 

                    <h5>District Activities</h5>
                        ...
                        <table class="table table-bordered">
                            <thead>
                                <tr>
                                    ...
                                    <th>District</th>-->
                                    <?php
                                        $database->query("SELECT activity_type_id,activity_type FROM rollout_activitytype WHERE activity_type_id NOT IN('1','4','5') ORDER BY activity_type_id ASC");
                                        $results = $database->statement->fetchall(PDO::FETCH_ASSOC);
                                        foreach ($results as $result) {
                                    ...
                                    <form action="<?php basename( $_SERVER['REQUEST_URI'] ) ?>" method="post"> 
                                        <?php 
                                            foreach ($results as $result) {
                                                echo 
                                                '<td class="datepicker_input">
                                                    <input name="activity_' . $result['activity_type_id'] . '_start" id="activity_' . $result['activity_type_id'] . '_start" type="text" placeholder="Start Date"/>
                                                    &nbsp;-&nbsp;
                                                    <input name="activity_' . $result['activity_type_id'] . '_end" id="activity_' . $result['activity_type_id'] . '_end" type="text" placeholder="End Date"/>
                                                </td>';
                                            }
                                            unset($results);
                                        ?>
                                        <td>
                                            ...
                                            <button type="submit" class='btn btn-primary' name="add_rollout_activity"><span class="glyphicon glyphicon-floppy-disk"></span></button> </td>
                                    </form>
                                </tr>
                            </tbody>
                    ...
                    <?php }else{echo '<p class="bg-info">You Have Insufficient Privileges to perform this Action</p>';}?>
                    <hr>
                    ...
                    <form method="post" action="<?php basename( $_SERVER['REQUEST_URI'] ) ?>">
                        <div class="clearfix">
                            <div class="pull-left" style="width:96%;background-color:#fff;padding:10px 20px;">
                                <div class="pull-left">
                                    <label>Sub County Training</label>
                                    <div class="clearfix">
                                        <input name="dt_start" id="dt_start" type="text" placeholder="Start Date" class="pull-left date" style="height:25px; padding-left: 5px;">
                                        <input name="dt_end" id="dt_end" type="text" placeholder="End Date" class="pull-left date" style="height:25px; padding-left: 5px; margin: 0px 0 0 10px;" >
                                    </div>
                                </div>
                                <div class="pull-left" style="margin-left:20px;">
                                    <label>Teacher Training</label>
                                    <div class="clearfix">
                                        <input name="tt_start" id="tt_start" type="text" placeholder="Start Date" class="pull-left date" style="height:25px; padding-left: 5px;">
                                        <input name="tt_end" id="tt_end" type="text" placeholder="End Date" class="pull-left date" style="height:25px; padding-left: 5px; margin: 0px 0 0 10px;" >
                                    </div>
                                </div>
                                <button type="submit" name="bulk_district_dates" class="btn btn-blck btn-primary pull-left" style="margin: 20px 0 0 10px;" >Set For Selected Sub Counties</button>
                            </div>
                        </div>
                        <br>
                        <div id="rollout-activity-table" class="table-responsive rollout-activity">
                            <table class="table table-bordered table-striped">
                                <thead>
                                    <tr>
                                        ...
                                        <th>Sub County</th>
                                        <?php
                                            $database->query("SELECT activity_type FROM rollout_activitytype ORDER BY activity_type_id ASC");
                                            $results = $database->statement->fetchall(PDO::FETCH_ASSOC);
                                            foreach ($results as $result) {
                                                echo '<th>' . $result['activity_type'] . '</th>';
                                        }
                                ...
                                <tbody>
                                    <?php
                                        ...
                                        <tr>
                                            <td>
                                                <input type="checkbox" value="<?php echo $result['activity_venu']; ?>" name="check[]">
                                            </td> 
                                            <?php
                                                //echo '<td>' . $result['actyvity_county'] . '</td>';
                                                echo '<td>' . $result['activity_venu'] . '</td>';

                                                $database->query('SELECT activity_type,start_date,end_date FROM rollout_activity WHERE activity_venu = :activity_venu AND wave_id = :wave_id ORDER BY activity_type ASC', array(
                                            ...
                                            <?php if( $priv_rap >=2 ) { ?>
                                                <td>
                                                    <a href="?act=editactivity&amp;wave=<?php echo $result['wave_id']; ?>&amp;loc=<?php echo urlencode($result['activity_venu']); ?>"class='btn btn-default' name="add_rollout_activity"><span class="glyphicon glyphicon-pencil"></span></a> 
                                                </td>
                                            <?php } ?>
                                        </tr>
                                    <?php $i++;} ?>
                ...
                <?php
                    ...
                    <div id="activity-color-key-container" class-"clearfix">
                        ...
                        <ul class="list-unstyled list-inline pull-left" id="activity-color-key">
                            <li><span class="key-box activity-type-1"></span> - County Meetings Planning</li>
                            <li><span class="key-box activity-type-2"></span> - Stakeholders Meeting</li>
                            <li><span class="key-box activity-type-3"></span> - CSHCC</li>
                            <li><span class="key-box activity-type-4"></span> - Sub county Training</li>
                            <li><span class="key-box activity-type-5"></span> - Teacher Training</li>
                            <li><span class="key-box activity-type-6"></span> - Deworming Day</li>
                        </ul>
                    </div>
                                    ...
                                    <div class='form-group clearfix'>
                                        ...
                                        <div class='col-md-6'>
                                            <input 
                                                ...
                                                type='text' 
                                                <?php if ( $result['activity_type'] == 1 || $result['activity_type'] == 2 ) { echo 'disabled'; } ?> 
                                            />
                                            &nbsp;-&nbsp;
                                            <input 
                        ...
                        <tbody>
                            ...
                            <?php
                                ...
                                foreach ($results as $key => $value) {
                                    ...
                                    <!-- <div class="clearfix">
                                        <h5 class="pull-left"> <?php //echo $theDistrict; ?></h5>
                                        <?php //if($priv_mt>=3){ ?>    
                                            <a href="?act=addtrainers&amp;waveid=<?php //echo $_REQUEST['waveid']; ?>&amp;loc=<?php //echo urlencode($value["activity_venu"]); ?>" class="btn pull-right"><span class="glyphicon glyphicon-pencil"></span></a>
                                        <?php //} ?>
                                    </div> -->
        ...
        <div id="add-trainers-container">
            ...
            <?php
                ...
                if ( !empty($results[0]['start_date']) || $results[0]['start_date'] != NULL ) {
                    ...
                    if (isset($_REQUEST['trainersData'])) {$rollOut->addMasterTrainers(explode(",", $_REQUEST['trainersData'])); }
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        function initDateoicker(parent) {
            var inputId = '#' + parent.attr('id');
