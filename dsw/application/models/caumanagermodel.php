...
class caumanagermodel extends Database {
    ...
    public function getRequiredCau($currentLevel){
        //Get All CAU above this one
    ...
    public function getTerritorryPrimeMeta($admin_territory_id, $country) {

        $data = array();
        
        $query = 'SELECT territory_level FROM admin_territory WHERE id = ' . $admin_territory_id . ' AND country_id = ' . $country . '';
        $territorry = $this->selectDBraw($query);
        $admin_territory_id_parent_parent = $territorry[0]['territory_level'] - 2;

        if ($admin_territory_id_parent_parent > 1) {
            $query = 'SELECT territory_name,territory_level FROM admin_territory WHERE territory_level = ' . $admin_territory_id_parent_parent . ' AND country_id = ' . $country . ' ORDER BY territory_level DESC LIMIT 1';
            $parent = $this->selectDBraw($query);
            if (!empty($parent)) {
                $data['prime_parent_territory_name'] = $parent[0]['territory_name'];
                $data['prime_parent_territory_level'] = $parent[0]['territory_level'];
            }
        }
    ...
    public function getTerritorryPrimeParents($admin_territory_id, $country) {
        
        $query = 'SELECT territory_level FROM admin_territory WHERE id = ' . $admin_territory_id . ' AND country_id = ' . $country . '';
        $this_level = $this->selectDBraw($query)[0]['territory_level'] - 2;
        if ($this_level > 1) {

            $query = 'SELECT id FROM admin_territory WHERE country_id = ' . $country . ' AND territory_level = ' . $this_level . '';
            $the_parent_id = $this->selectDBraw($query);
            if (!empty($the_parent_id)) {
                $the_parent_id = $this->selectDBraw($query)[0]['id'];
                $query = 'SELECT id, admin_territory_name FROM admin_territory_details WHERE admin_territory_id = ' . $the_parent_id . ' ORDER BY admin_territory_name ASC';
    ...
    public function delTerritorry($id) {
        ...
        $query = 'SELECT * FROM admin_territory_details WHERE territory_parent = ' . $id . '';
        ...
        $admin_territory_id =  $ter_child1[0]['admin_territory_id'];
        $admin_territory_name = $ter_child1[0]['admin_territory_name'];

            $query = 'SELECT territory_name FROM admin_territory WHERE id = ' . $admin_territory_id . '';
        ...
        foreach ($data as $key => $datum) {
            ...
            foreach ($datum as $key_1 => $value) {
                ...
                if (empty($value)) {
                    ...
                    $theID = $this->selectDB('admin_territory_details', array('id'), array('admin_territory_id' => $admin_territory_id, 'admin_territory_name' => $admin_territory_name, 'territory_parent' => $territory_parent ));
                    $count = $this->count();
