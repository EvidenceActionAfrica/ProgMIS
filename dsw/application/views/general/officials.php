                    ...
                    <?php
                        ...
                        foreach ($data as $key => $value) { ?>
                            ...
                            <td class="index"></td>
                                ...
                                // $i = 1;
                                ?>
                                <td><a href="<?php echo URL ?>generalclass/updateOfficials/<?php echo $table . "/" . $data[$i]['id']; ?>" class="btn btn-default btn-xs">Edit</a>
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title" id="myModalLabel">Add Official</h4>
            </div>
            <form  action="<?php echo URL; ?>generalclass/addOfficial/<?php echo $table; ?>" data-async data-target="myModal" method="post" role="form" id="modal-form">
                        ...
                        <div class="col-md-12">
                            <?php
                                foreach ($fields as $key => $value) {
                                    ...
                                    } else if ($value['Key'] == 'MUL' && $value['Field'] != 'country' && $value['Field'] != 'waterpoint_id'&& $value['Field'] != 'title') {
                                    ...
                                    } else if ($value['Key'] == 'MUL' && $value['Field'] == 'title' ) {
                                        echo '<div class="form-group">
                                                <label>' . ucwords(str_replace('_', ' ', $value['Field'])) . '</label><br>
                                                <select name="' . $value['Field'] . '" class="form-control input-sm" required>
                                                    <option value="">Select ' . ucwords(str_replace('_', ' ', $value['Field'])) . '</option>';
...
<div class="modal fade" id="myImportModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            ...
            <form  action="<?php echo URL; ?>importclass/importOfficials/<?php echo $table.'/generalclass/general'; ?>" enctype="multipart/form-data" data-async data-target="myModal" method="post" role="form" id="modal-form">
                <div class="modal-body">
                    <div id="message"></div>
                    <div class="row">
                        <div class="col-md-12">
...
<script type="text/javascript">
    ...
    function show_confirm(tables, deleteId) {
        if (confirm("Are you sure you want to delete?")) {
            location.replace('<?php echo URL ?>generalclass/deleteOfficials/' + tables + '/' + deleteId);
            console.log('<?php echo URL ?>generalclass/deleteOfficials/' + tables + '/' + deleteId);
         } else {
            console.log('<?php echo URL ?>generalclass/deleteOfficials/' + tables + '/' + deleteId);
            return false;
        }
