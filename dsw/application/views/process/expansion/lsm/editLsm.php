...
<div id="myModal" class="col-md-6 col-md-offset-3" >
    <div >
        <div >
            ...
            <form  action="<?php echo URL; ?>expansion/updateLsm/<?php echo $table . '/' . $editLsm . '/' . $extraUrl; ?>" data-async data-target="myModal" method="post" role="form" id="modal-form">
                        ...
                        <div class="col-md-12">
                            ...
                            foreach ($fields as $key => $value) {
                                ...
                                if ($value['Field'] == 'lsm_id') {
                                    ...
                                    echo '<input type="hidden" value="' . $_SESSION['country'] . '"  name="' . $value['Field'] . '" class="form-control input-sm" readonly/>
                                    ';
                                    ...
                                    if (!isset($single_record)) {
                                        ...
                                        echo '
                                                 <input type="hidden" name="' . $value['Field'] . '" class="form-control input-sm" value="" />
                                        ...
                                        echo '
                                            <div class="form-group">
                                                <label> Territoty Level</label><br>
                                                <input type="text" name="" class="form-control input-sm" value="' . ucwords(str_replace('_', ' ', $territoryselected[0]["territory_name"])) . '" readonly/>
                                                                                                    

                                            </div>
                                        <div class="form-group">
                                            <label> ' . ucwords(str_replace('_', ' ', str_replace('id', 'name', $value['Field']))) . '</label><br>
                                            <select id="' . $value['Field'] . '" name="' . $value['Field'] . '" class="form-control input-sm" required>
                                            ';
                                        foreach ($cauSelect as $key => $value_) {
                                            if($value_['id']==$single_record[$x]){
                                                 echo'<option value="' . $value_['id'] . '" selected>' . ucwords(str_replace('_', ' ', $value_["admin_territory_name"])) . '</option>';
                                            }else{
                                                echo'<option value="' . $value_['id'] . '" >' . ucwords(str_replace('_', ' ', $value_["admin_territory_name"])) . '</option>'; 
                                            }
                                        }
                                        echo '</select>
                                        </div>
                ...
                <div class="col-md-offset-4">
                    ...
                    <a href='<?php echo URL . "expansion/viewLsm/?tabActive=tab2" ?>'>
                        <button type="button" class="btn btn-default" data-dismiss="modal">Cancel</button>
                    </a>
                    <button  type="submit" class="btn btn-primary" name="update-lsm" id="update-lsm">Update Details</button>
                </div>
            </form>
        </div>
    </div>
