...
<div id="myModal" class="col-md-6 col-md-offset-3" >
    <div >
        <div >
            ...
            <form  action="<?php echo URL; ?>chlorineclass/update/<?php echo $table ?>" data-async data-target="myModal" method="post" role="form" id="modal-form">
                        ...
                        <div class="col-md-12">
                            ...
                            foreach ($fields as $key => $value) {
                                ...
                                if ($value['Key'] == 'PRI') {
                                    ...
                                    echo '<input type="hidden" name="'.$value['Field'].'" value="' . $single_record[$x] . '"/>';

                                } else if ($value['Field'] == 'last_update') {

                                } else if (strpos($value['Field'], 'phone') !== false || strpos($value['Field'], 'contact') !=false) {
                                    if (!isset($single_record)) {
                                        echo '
                                                <div class="form-group">
                                                    <label>' . ucwords(str_replace('_', ' ', $value['Field'])) . '</label><br>
                                                    <input type="text" name="" class="form-control input-sm" value="" onKeyUp="isNumeric(this.id);"/><span id="' . $value['Field'] . 'Span"></span>
                                                </div>
                                            ';
                                    } else {
                                        echo '
                                                <div class="form-group">
                                                    <label>' . ucwords(str_replace('_', ' ', $value['Field'])) . '</label><br>
                                                    <input type="text" value="' . $single_record[$x] . '"  name="' . $value['Field'] . '" class="form-control input-sm" onKeyUp="isNumeric(this.id);"/><span id="' . $value['Field'] . 'Span"></span>
                                                </div>
                                            ';
                                    }
                ...
                <div class="col-md-offset-4">
                    ...
                    <a href='<?php echo URL . "chlorineclass/chlorine/"  ?>'>
                        <button type="button" class="btn btn-default" data-dismiss="modal">Cancel</button>
                    </a>
                    <button  type="submit" class="btn btn-primary" name="update-chlorine-data" id="update-chlorine-data">Update Details</button>
                </div>
            </form>
        </div>
    </div>
