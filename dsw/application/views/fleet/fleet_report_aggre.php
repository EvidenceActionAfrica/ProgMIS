...
<div class="col-md-10">
    ...
    <div id="data-table-manger">
        ...
        <div class="clearfix">
            <h3 class="pull-left">Motor<?php echo $category; ?> Aggregate Report</h3>
        </div>
        <hr>
    </div>
    <form method="$_GET" >
        <div class="table-responsive">
            <table id="data-table" class="table table-striped table-hover">
                ...
                <thead>                               
                    <tr>
                        <th class="export-visible" colspan="8" style="border-radius: 5px 5px 0px 0px; ">  
                            ...
                            <select class="form-control input-sm" style="width:110px; float: left;"  name="choose_year" id="choose_year">
                                ...
                                <?php
                                foreach ($dataYearLogMaint as $key => $value) {
                                    ?>
                                    <option value="<?php echo $value['Year']; ?>"<?php if ($year == $value['Year']) echo 'selected'; ?>>
                                        <?php echo $value['Year']; ?></option>
                                <?php } ?>
                            </select>
                            <input class="btn btn-default pink-button" style="height: 30px; margin-left: 5px; margin-right: 20px;" type="submit" name="submit_year"  value="Submit">
                            <?php if ($year != 'all') { ?>
                    ...
                    <?php if ($year != 'all') { ?>
                        <tr>
                            <th class="export-visible" colspan="8">
                                <?php if (!isset($_GET['alter_week']) && !isset($_GET['submit_week'])) { ?>
                                    ...
                                    <?php if ($month != 'all') { ?>
                                        ...
                                        <select class="form-control input-sm" style="width:75px; float: left;"  name="choose_day" id="choose_day">
                                            <option value='all' <?php echo 'selected'; ?>>None</option>
                                            <?php
                                            foreach ($dataDayLogMaint as $key => $value) {
                                                ?>
                                ...
                                <?php } else { ?>
                                    ...
                                    <select class="form-control input-sm" style="width:110px; float: left;"  name="choose_week" id="choose_week">
                                        <option value='all' <?php echo 'selected'; ?>>None</option> 
                                        <?php
                                        foreach ($dataWeekLogMaint as $key => $value) {
                                            ?>
                    ...
                    <?php } ?>
                    <tr > 
                        <th class="export-visible">Reg Num</th>
                        <th class="export-visible">Maintenance Cost</th>
                        <th class="export-visible">Fuel Cost</th>
                        <th class="export-visible">Total Cost</th>
                        <th class="export-visible">Distance Covered</th>
                        <th class="export-visible">Fuel Usage</th>
                        <th class="export-visible">Distance Covered Per Liter</th>
                        <th class="export-visible">Cost Per Kilometer</th>
                    </tr>
                </thead>
                <tbody>
                    <?php
                    foreach ($data as $key => $value) {
                        ?>
                        <tr>
                            <td><?php echo $reg_num = $value["RegNum"]; ?></td>
                            <?php $data_maint = $fleetlistmodel->getDataMaintOnly($categ, $dayComplex, $weekComplex, $monthComplex, $yearComplex, $value["RegNum"]); ?>
                            <td><?php echo $val_maint_cost = $data_maint[0]['total_maint']; ?></td>
                            <?php $data_log = $fleetlistmodel->getDataLogOnly($categ, $dayComplex, $weekComplex, $monthComplex, $yearComplex, $value["RegNum"]); ?>
                            <td><?php echo $val_fuel_cost = $data_log[0]['total_Fuel']; ?></td>
                            <td><?php echo $val_total_cost = $val_maint_cost + $val_fuel_cost; ?></td>
                            <td><?php echo $val_dist_cov = $data_log[0]['total_Kilometer']; ?></td>
                            <td><?php echo $val_fuel_quant = $data_log[0]['total_FuelQuant']; ?></td>
                            <td><?php
                                if ($val_fuel_quant != 0) {
                                    echo $val_dist_cov_per_litre = round(($val_dist_cov / $val_fuel_quant), 2);
                                }
                                ?></td>
                            <td><?php
                                if ($val_dist_cov != 0) {
                                    echo $val_cost_per_kil = round(($val_total_cost / $val_dist_cov), 2);
                                }
                                ?></td>
                        </tr>
                        <?php
    ...
    $(document).ready(function() {
        ...
        var table = $('#data-table').DataTable({
            ...
            "footerCallback": function(row, data, start, end, display) {
                ...
                // Total over all pages column 7
                pageTotal7 = api
                        .column(7, {filter: 'applied', order: 'current'})
                        .data()
                        .reduce(function(a, b) {
                    return intVal(a) + intVal(b);
                }, 0);
