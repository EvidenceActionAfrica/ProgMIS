...
class expansion extends Controller {
    ...
    private function fieldsArray($tableCall) {
        ...
        switch ($tableCall) {
            ...
            case 'tid':
                return $fieldsArray = array('id', 'program', 'waterpoint_id', 'verification_id', 'installation_date', 'csa_responsible', 'field_officer', 'was_it_installed', 'were_materials_mobilized', 'problems_with_installation');
            case 'tracking_waterpoint_data':
                return $fieldsArray = array('id', 'country', 'program', 'village', 'waterpoint_name', 'waterpoint_id', 'verification_id', 'land_owner_name', 'land_owner_contact', 'neighbor_name', 'neighbor_contact', 'number_of_hhs');
    ...
    private function integrityChecks($uncheckedData) {
        foreach ($uncheckedData as $key => $value) {
            ...
            $odkarraywaterpoint = $this->odkDataCall($match[0]['api_key'], $match[0]['column_name'], $value['verification_id']);

            if (is_array($odkarraywaterpoint)) {
                ...
                $uncheckedData[$key]['reverification'] = "YES";
                unset($uncheckedData[$key]['Match']);
            } else if ($odkarraywaterpoint == null) {
    ...
    private function odkDataCall($spreadsheet_key, $column_to_use, $checkValue) {
                ...
                if (isset($someArray['table']['rows'][0]['c']['0']['v'])) {
                    for ($i = 0; $i < $numberOfRows; $i++) {
                        ...
                        if ($checkValue == $someArray['table']['rows'][$i]['c']['0']['v']) {
                            $waterpoint_id = $someArray['table']['rows'][$i]['c']['0']['v'];
                            if (sizeof( $someArray['table']['rows'][$i]['c'])>1){
    ...
    public function verificationTrackAdd($table, $program) {
        ...
        if (isset($_POST['add-verification-data'])) {
            ...
            $tableName = str_replace("_", " ", $table);
            $tableName = ucwords($tableName);
            $uncheckedData = $expansionmodel->getVerificationWHEREData($table, $fieldsArray, 'program', $_POST['program']);
            $fields = $expansionmodel->getFields($table, $fieldsArray);
            $programDropDown = $this->getProgramDropDown();
            $staffDropDown = $this->getStaffDropDown();
            $data = $this->integrityChecks($uncheckedData);
            $odkData = $expansionmodel->getOdkData('odk_verification', $program);
    ...
    public function uploadPasslist($table) {
        ...
        if (isset($_POST['confirmWaterpoints'])) {
            ...
            foreach ($data as $key => $value) {
                ...
                if ($status != "FAIL" && $status != "fail" && $status != "NULL" && $status != "null" && !empty($status) && $value['waterpoint_id'] != '' && $villageId != null) {
    ...
    private function getVcsOdkResult($data, $program) {
        ...
        if (isset($result)) {
            foreach ($data as $key => $value) {
                $response = $this->odkDataCall($result[0]['api_key'], $result[0]['column_name'], $value['village']);
                if (is_array($response) && $response != "401x" && $response != "404x") {
                    $data[$key]["vcs_odk_on_server"] = 'YES';
                } else if ($response == "401x") {
                    $data[$key]["vcs_odk_on_server"] = 'Column Not Found';
                } else if ($response == "404x") {
    ...
    public function vcsVerificationTrackingAdd($table) {
        ...
        if (isset($_POST['add-vcs-data'])) {
            ...
            $cauManage = $scheduleModel->checkSelectedCau('vcs_gen_schedule');
            $odkData = $expansionmodel->getOdkData('odk_vcs', $program);
            $data = $expansionmodel->getVcsWHEREData($table, $fieldsArray, 'program', $_POST['program']);
            $fields = $expansionmodel->getFields($table);
            require 'application/views/_templates/header.php';
            require 'application/views/process/expansion/sidebar.php';
            require 'application/views/process/expansion/vcs/vcsTrack2.php';
