...
$query = mysqli_query($mysqli, "SELECT distrcit,program_name,waterpoint_name,waterpoint_id, carbon_status,longitude,latitude FROM dispenser_database
...
while ($waterpoints = mysqli_fetch_assoc($query)) {
   $js.='{lat:'.$waterpoints['latitude'].',lng:'.$waterpoints['longitude'].',data:{drive:false,zip:"waterpoint: '.mysqli_real_escape_string($mysqli,$waterpoints['waterpoint_name']).'",district:"district: '.mysqli_real_escape_string($mysqli,$waterpoints['district']).'"}},' ;
}
$js=rtrim($js,',');
