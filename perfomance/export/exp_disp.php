...
$field2 = 's209_dispprob_cas_padlk';
...
while ($row = mysqli_fetch_assoc($res2)) {
    ...
    $s = 'b';
    for ($value = 1; $value < 13; ++$value) {
        $query = "SELECT $field2 FROM dsw_per_dispensed_rates WHERE month = '$value' AND program = '$prog' AND ($field2 = '1' OR s210_dispprob_tnk_vlv ='1' OR s212_nut ='1') $year_sub";
        $result = mysqli_query($mysqli, $query) or die(mysqli_query($mysqli));
        $nume = mysqli_affected_rows($mysqli);
        ...
        $objPHPExcel->setActiveSheetIndex(0)
                ->setCellValue('A' . $dj, 'Hardware Problem')
                ->setCellValue('A' . $j, '')
    ...
    $query = "SELECT $field2 FROM dsw_per_dispensed_rates WHERE program = '$prog' AND ($field2 = '1' OR s210_dispprob_tnk_vlv ='1' OR s212_nut ='1') $year_sub";
    $result = mysqli_query($mysqli, $query) or die(mysqli_query($mysqli));
    $nume = mysqli_affected_rows($mysqli);
...
$ma2 = 'b';
for ($value = 1; $value < 13; ++$value) {
    $query = "SELECT $field2 FROM dsw_per_dispensed_rates WHERE country='$country_val' AND  month = '$value' AND ($field2 = '1' OR s210_dispprob_tnk_vlv ='1' OR s212_nut ='1') $year_sub";
    $result = mysqli_query($mysqli, $query) or die(mysqli_query($mysqli));
    $nume = mysqli_affected_rows($mysqli);
...
// Rename worksheet
$today = date("F j-Y");
$objPHPExcel->getActiveSheet()->setTitle(' Dispenser Rates');
...
header('Content-Disposition: attachment;filename=" Dispenser Rates.xlsx"');
